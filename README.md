...
## بسته‌های آماده و آرشیو کارت‌ها

- در بخش `کارت‌ها` داخل اپ، سه زیرتب وجود دارد: `فعال`، `آرشیو` و `بسته‌ها`.
- کاربر می‌تواند کارت‌های فعال را آرشیو کند؛ کارت آرشیوشده در مرور روزانه، آمار لایتنر و سقف کارت پلن حساب نمی‌شود، ولی قابل بازگردانی است.
- تب `بسته‌ها` پکیج‌های منتشرشده ادمین را نشان می‌دهد. کاربر می‌تواند مثلا امروز ۱۰ کارت از یک بسته و ۵ کارت از بسته دیگر اضافه کند.
- کارت تکراری از همان بسته دوباره اضافه نمی‌شود. اگر دو بسته کارت مشابه داشته باشند، اپ با امضای `نوع کارت + روی کارت` جلوی تکرار را می‌گیرد.
- یک بسته نمونه با شناسه `essential-50` شامل ۵۰ واژه کاربردی انگلیسی به صورت پیش‌فرض seed می‌شود.
- در پنل `/admin` بخش `بسته‌های آماده` برای ساخت، ویرایش، حذف، انتشار/پیش‌نویس کردن و محدود کردن بسته به پلن‌های خاص اضافه شده است.
- فیلد `پلن‌های مجاز` اگر خالی باشد یعنی همه پلن‌ها دسترسی دارند؛ اگر مثلا `Pro, Plus` باشد، کاربران پلن‌های دیگر بسته را خاکستری و قفل‌شده می‌بینند.
- کارت‌های بسته در ادمین به شکل JSON ویرایش می‌شوند تا ساخت بسته‌های بزرگ با AI یا فایل CSV راحت باشد.
...
پس از اولین ورود آنلاین، PWA فایل‌های برنامه و آخرین نسخه کارت‌های همان حساب را در دستگاه نگه می‌دارد. در حالت آفلاین کاربر می‌تواند:

- کارت‌های فعال و آرشیوشده را بخواند.
- کارت دستی اضافه یا ویرایش کند.
- کارت را آرشیو، فعال یا حذف کند.
- مرور روزانه را انجام دهد و جعبه لایتنر را جلو ببرد.

تغییرات آفلاین در IndexedDB و جداگانه برای هر حساب ذخیره می‌شوند. نشانگر بالای صفحه تعداد عملیات منتظر را نمایش می‌دهد و با برگشت اینترنت، صف به‌ترتیب با سرور همگام می‌شود. ساخت کارت دارای شناسه ثابت است و سینک وضعیت مرور نیز idempotent طراحی شده تا retry باعث کارت یا مرور تکراری نشود.

قابلیت‌های AI، دیکشنری هوشمند، اصلاح متن، فعال‌سازی کد و دریافت بسته در حالت آفلاین در دسترس نیستند. برای آماده‌شدن حالت آفلاین، کاربر باید حداقل یک بار بعد از ورود، برنامه را آنلاین باز کرده باشد.
