...
مینی‌اپ و PWA برای یادگیری زبان با لایتنر، کارت فیدبک، دیکشنری هوشمند، اصلاح و تحلیل متن، خروجی/ورودی JSON، ربات تلگرام و پنل ادمین جدا.
...
برای ورود نسخه مرورگر/PWA:

1. کاربر ربات را start می‌کند.
2. دستور `/login` را می‌زند.
3. کد ۶ رقمی را در صفحه ورود PWA وارد می‌کند.
4. از آن به بعد همان اکانت تلگرام روی مرورگر یا نسخه نصب‌شده باز می‌شود.
...
اگر اپ خارج از تلگرام باز شود و `TELEGRAM_BOT_TOKEN` روی سرور فعال باشد، اپ صفحه اتصال به ربات را نشان می‌دهد و با کد `/login` یک session پایدار برای همان اکانت تلگرام می‌سازد.
...
- نسخه PWA با manifest و service worker
- آمار ادمین برای کارت‌ها، فعالیت امروز، کارت‌های افزوده‌شده، مرورها و مصرف AI هر کاربر
